...
class ListsSuite extends FunSuite {
  ...
  test("sum of a numbers, some negative") {
    assert(sum(List(1,2,0, -3, 5, -2)) === 3)
  }
  test("sum of a negative numbers") {
    assert(sum(List(-1,-2,0)) === -3)
  }
  test("sum of a equal numbers") {
    assert(sum(List(2,2,2,3,3,3)) === 15)
  }
  ...
  test("Max of list with negative numbers") {
	  assert(max(List(-2,-51,-3)) === -2)
  }
  
  test("Max of list with negative and positive numbers") {
	  assert(max(List(-2,-51, 1,-3, 3, 0)) === 3)
  }
  
  test("Max of list with single element") {
	  assert(max(List(-2)) === -2)
  }
}
